...
export default function BecomePartnerPage() {
  ...
  const fetchProfile = async () => {
    try {
      const { data } = await axios.get(
        `${API}/api/delivery-partners/mine`,
        {
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await axios.get(
        `${API}/api/delivery-partners/my-orders`,
        {
          withCredentials: true,
        },
      );
  ...
  const handleGenerateOtp = async (orderId) => {
    ...
    try {
      const { data } = await axios.post(
        `${API}/api/delivery-partners/orders/${orderId}/generate-otp`,
        {},
        { withCredentials: true },
      );
      setOtp(orderId, {
  ...
  const handleVerifyOtp = async (orderId) => {
    ...
    try {
      await axios.post(
        `api/delivery-partners/orders/${orderId}/verify-otp`,
        { otp: s.inputOtp.trim() },
        { withCredentials: true },
      );
      setOtpState((p) => {
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Earn by delivering FreshCart orders
            </p>
          </div>
        </div>
