  ...
  const loadPartners = async () => {
    ...
    try {
      const pincode =
        ...
        order.shippingAddress?.pincode ||
        "";
      const url = `const API = import.meta.env.VITE_API_URL;/api/delivery-partners/admin/active${pincode ? "?pincode=" + pincode : ""}`;
...
const AdminOrderDetails = () => {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      ...
      const { data } = await axios.get(
        `const API = import.meta.env.VITE_API_URL;/api/orders/admin/${id}`,
        { withCredentials: true },
      );
      setOrder(data);
    } catch {
  ...
  const updateOrderStatus = async (action) => {
    try {
      ...
      const url =
        action === "cancel"
          ? `const API = import.meta.env.VITE_API_URL;/api/orders/admin/${id}/cancel`
          : `const API = import.meta.env.VITE_API_URL;/api/orders/${id}/${action}`;
