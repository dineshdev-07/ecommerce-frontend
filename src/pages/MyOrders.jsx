...
const MyOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const { data } = await axios.get(
        "http://const API = import.meta.env.VITE_API_URL;/api/orders/myorders",
        {
  ...
  const cancelOrderHandler = async (orderId) => {
    ...
    try {
      ...
      await axios.put(
        `http://const API = import.meta.env.VITE_API_URL;/api/orders/${orderId}/cancel`,
        {},
        {
          withCredentials: true,
        },
  ...
  const downloadInvoice = async (orderId) => {
    try {
      const response = await fetch(
        `http://const API = import.meta.env.VITE_API_URL;/api/orders/${orderId}/invoice`,
        {
          credentials: "include",
        },
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 md:p-8 font-sans animate-fadeIn rounded-2xl border border-gray-300 shadow-sm">
