...
function AssignModal({
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const url =
          lat && lng
            ? `const API = import.meta.env.VITE_API_URL;/api/delivery-partners/admin/active?lat=${lat}&lng=${lng}${pincodeParam}${districtParam}`
            : `const API = import.meta.env.VITE_API_URL;/api/delivery-partners/admin/active?${pincodeParam.slice(1)}${districtParam}`;
  ...
  const handleAssign = async () => {
    ...
    try {
      await axios.put(
        "const API = import.meta.env.VITE_API_URL;/api/delivery-partners/admin/orders/assign",
  ...
  return (
    ...
    >
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-slideUp">
        <div className="flex items-center justify-between px-5 rounded-2xl border border-gray-300 shadow-sm py-4 border-b  bg-gray-50">
          <div>
...
function UnassignModal({ order, onClose, onDone }) {
  ...
  const handleUnassign = async () => {
    ...
    try {
      await axios.put(
        `const API = import.meta.env.VITE_API_URL;/api/delivery-partners/admin/orders/${order._id}/unassign`,
        {},
        { withCredentials: true },
      );
      onDone();
...
function AdminOrders() {
  ...
  const deliverHandler = async (id) => {
    ...
    try {
      await axios.put(
        `const API = import.meta.env.VITE_API_URL;/api/orders/${id}/deliver`,
        {},
        { withCredentials: true },
      );
      fetchOrders();
  ...
  const refundHandler = async (id) => {
    ...
    try {
      await axios.put(
        `const API = import.meta.env.VITE_API_URL;/api/orders/${id}/refund`,
        {},
        { withCredentials: true },
      );
  ...
  const resetDatabaseHandler = async () => {
    ...
    try {
      await axios.delete(
        "const API = import.meta.env.VITE_API_URL;/api/orders/reset",
        {
          withCredentials: true,
        },
      );
  ...
  return (
    <div className="min-h-screen bg-[#F4F7F6] p-4 md:p-8 font-sans animate-fadeIn rounded-2xl border border-gray-300 shadow-sm">
      {showAssignModal && (
        <AssignModal
